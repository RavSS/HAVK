...
IS
   ...
   BEGIN
      ...
      THEN
         Log("The FAT file system does not use a sector size of 512.",
            warning);
         Log("Cannot continue as of this time.", warning);
      ...
      THEN
         Log("File system on EFI drive is not FAT12 or FAT16 (likely FAT32)," &
            " cannot proceed as of now.", warning);
      ...
      THEN
         Log("FAT file system found.", nominal);
      ELSE
         Log("FAT file system found, but it could be corrupt.", warning);
      END IF;
      ...
      IF
         FAT_Version /= FAT16
      THEN
         Log("Can only parse FAT16. Cannot proceed.", warning);
   ...
   BEGIN
      ...
      IF
         Next_Cluster = No_File_Match
      THEN
         Log("Could not obtain file: " & Path_Name, warning);
         RETURN;
      END IF;
      ...
      LOOP
         ...
         Search_For_Entry(FAT_Context, Parsed_Path(Path_Index).Name,
            Next_Cluster, Next_Cluster, Next_Entry,
      ...
      Log(Image(Next_Entry.File_Size), warning);

      IF
        (Shift_Left(Next_Entry.Cluster_High, 16) OR Next_Entry.Cluster_Low) <
            invalid_cluster_16'first AND THEN
         Next_Entry.File_Size /= 0 AND THEN
         Base_Byte <= Next_Entry.File_Size AND THEN
         Byte_Size <= (Next_Entry.File_Size - Base_Byte) + 1
      THEN
         File_To_Memory(FAT_Context, Next_Entry, Destination,
            Base_Byte, Byte_Size);
      ELSE
         Log("Byte offsets incorrect for " & Path_Name & '.', warning);
      END IF;
   END Read_File;
