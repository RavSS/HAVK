...
IS
   ...
   -- The main type for the package. The fields are hidden and do not need to
   -- be seen by other kernel code.
   ...
   -- a specific access type in one of its parameters.
   GENERIC
   ...
   -- was freed; otherwise, it was not deallocated.
   GENERIC
      TYPE generic_data IS PRIVATE;
      TYPE generic_pointer IS ACCESS generic_data;
   ...
   -- This is used for testing memory allocations and deallocations. It is not
   -- strenuous, it's very basic and does not guarantee error-free management.
...
PRIVATE
   ...
   -- The internal memory space for the kernel heap. Only this package should
   -- touch it, as it is specially handled.
   Kernel_Heap : space
   WITH
