...
IS
      ...
      CASE PS2.Input_Controller.Current_Scancode_Set IS
         ...
         WHEN set_2 => -- TODO: Set 2 is mostly implemented.
            IF Scancode = 16#F0# THEN
               ...
               Break_State := true;
               RETURN;
            ELSIF Scancode = 16#12# OR ELSE Scancode = 16#59# THEN
               -- Shift logic.
               Shift_State    := NOT Break_State;
               Last_Key_State := Scancode_Set_2(Scancode);
            ELSIF Scancode = 16#58# THEN
               -- Caps lock logic.
               IF NOT Break_State THEN
                  Caps_Lock_State := NOT Caps_Lock_State;
               END IF;
               Last_Key_State := Scancode_Set_2(Scancode);
            ELSE
               -- Standard key press logic. Resets break state.
               Last_Key_State := Scancode_Set_2(Scancode);
   ...
   BEGIN
      ...
      Key.Shifted   := (IF Caps_Lock_State THEN NOT Shift_State
         ELSE Shift_State);
