...
IS
   ...
   BEGIN
      ...
      FOR RSDP_Byte OF RSDP_Bytes LOOP
         Total_Bytes := Total_Bytes + RSDP_Byte;
      END LOOP;

      IF (Total_Bytes AND 16#FF#) /= 0 THEN
         RETURN false;
      ELSE
         Total_Bytes := 0; -- Reset it to check the XSDT's byte sum.
      END IF;

      FOR XSDT_Byte OF XSDT_Bytes LOOP
   ...
   BEGIN
      FOR Table OF Tables LOOP
         IF Table.Signature = Signature THEN
            DECLARE
               ...
               Total_Bytes : num := 0;
            BEGIN
               FOR Table_Byte OF Table_Bytes LOOP
   ...
   IS
      ...
      Byte_Index   : num := APICs_Bytes'first;
      APIC_Index   : num := APICs'first;
   BEGIN
      WHILE -- Keep going until the indices go out of the array ranges.
              Byte_Index IN APICs_Bytes'range AND
         THEN APIC_Index IN       APICs'range -- Ignore any APICs above 255.
      LOOP
