...
IS
   ...
   TYPE system_description_table IS RECORD
      ...
      OEM_Revision   : num RANGE 0 .. 16#FFFFFFFF#;
      -- A numerical identity belonging to the utility that made the table.
      Maker          : num RANGE 0 .. 16#FFFFFFFF#;
      -- The revision number of the maker tool.
      Maker_Revision : num RANGE 0 .. 16#FFFFFFFF#;
   END RECORD
   WITH
      Predicate  => Length > 36, -- The SDT will never be alone in a table.
      Convention => C;
   ...
   TYPE system_description_tables IS ARRAY(num RANGE <>) OF
   ...
   END RECORD
   WITH
      Predicate  => SDT.Length >= 36 + 8, -- At least one pointer will exist.
      Convention => C;
   ...
   TYPE root_system_description_pointer IS RECORD
      ...
      RSDT_Address       : num RANGE 0 .. 16#FFFFFFFF#;
      ...
      Extra_Checksum     : num RANGE 0 .. 16#000000FF#;
      -- Reserved bytes.
      Reserved           : num RANGE 0 .. 16#00FFFFFF#;
   END RECORD
   WITH
      Convention => C;
   ...
   TYPE fixed_ACPI_description_table_feature_flags IS RECORD
      ...
      Reserved          : num RANGE 0 .. 16#000003FF#;
   END RECORD
   WITH
      Convention => C;
   ...
   TYPE fixed_ACPI_description_table IS RECORD
      ...
      FACS_Address_Low  : num RANGE 0 .. 16#FFFFFFFF#;
      -- The 32-bit physical address of the DSDT.
      DSDT_Address_Low  : num RANGE 0 .. 16#FFFFFFFF#;
      -- A reserved field. It was originally used in ACPI 1.0.
      Reserved_1        : num RANGE 0 .. 16#000000FF#;
      ...
      SCI_Vector        : num RANGE 0 .. 16#0000FFFF#;
      -- The I/O port of the system management interrupt (SMI) command port.
      SMI_Port          : num RANGE 0 .. 16#FFFFFFFF#;
      ...
      Enable_Value      : num RANGE 0 .. 16#000000FF#;
      ...
      PM1_Event_Size    : num RANGE 4 .. 16#000000FF#;
      -- Number of bytes decoded by the power management 1a event register
      -- block and the PM1b event register block if it is supported.
      PM1_Control_Size  : num RANGE 2 .. 16#000000FF#;
      -- Number of bytes decoded by the power management 2 event register
      -- block. If it is not supported, then this is zero.
      PM2_Control_Size  : num RANGE 0 .. 16#000000FF#;
      ...
      Cx_Value          : num RANGE 0 .. 16#000000FF#;
      ...
      C2_Shift_Latency  : num RANGE 0 .. 16#0000FFFF#;
      -- The worst-case latency of how long it takes to shift through
      -- the C3 state. If this is greater than 1000, then the system doesn't
      -- support the C3 state.
      C3_Shift_Latency  : num RANGE 0 .. 16#0000FFFF#;
      ...
      RTC_Century       : num RANGE 0 .. 16#000000FF#;
      ...
      Reserved_2        : num RANGE 0 .. 16#000000FF#;
   ...
   TYPE multiple_APIC_description_table IS RECORD
      ...
      LAPIC_Address     : num RANGE 0 .. 16#FFFFFFFF#;
      ...
      Reserved_Flags    : num RANGE 0 .. 16#7FFFFFFF#;
   ...
   TYPE local_APIC_descriptor IS RECORD
      ...
      ACPI_Identity     : num RANGE 00 .. 16#000000FF#;
      -- The (presumably physical) identity of the local APIC.
      APIC_Identity     : num RANGE 00 .. 16#000000FF#;
      ...
      Reserved_Flags    : num RANGE 00 .. 16#3FFFFFFF#;
   END RECORD
   WITH
      Predicate  => Enumeration_Value = local_APIC       AND THEN
                    Length = 8                           AND THEN
   ...
   TYPE local_x2APIC_descriptor IS RECORD
      ...
      Length            : num RANGE 00 .. 16#000000FF#;
      -- A reserved field.
      Reserved          : num RANGE 00 .. 16#0000FFFF#;
      -- The (presumably physical) identity of the local x2APIC.
      x2APIC_Identity   : num RANGE 00 .. 16#FFFFFFFF#;
      ...
      -- This is related to ACPI AML. OSPM must match each LAPIC (regardless of
      -- specification) to a processor.
      ACPI_Identity     : num RANGE 00 .. 16#FFFFFFFF#;
   END RECORD
   WITH
      Predicate  => Enumeration_Value = local_x2APIC     AND THEN
                    Length = 16                          AND THEN
                   (IF Enabled     THEN NOT Activatable) AND THEN
                   (IF Activatable THEN NOT Enabled),
      Convention => C;
   ...
   TYPE interrupt_controller_descriptor IS RECORD
      ...
      Record_Address    : System.Address       := 0;
   END RECORD;
   ...
   TYPE interrupt_controller_descriptors IS ARRAY(num RANGE 1 .. 255) OF
   ...
   TYPE bytes IS ARRAY(num RANGE <>) OF num RANGE 0 .. 16#FF#
   WITH
      Component_Size => 8;
   ...
   TYPE high_precision_event_timer_table IS RECORD
      ...
      Hardware_Revision : num RANGE 0 .. 16#00FF#;
      ...
      Comparators       : num RANGE 0 .. 16#001F#;
      ...
      Vendor_Identity   : num RANGE 0 .. 16#FFFF#;
      ...
      HPET_Sequence     : num RANGE 0 .. 16#00FF#;
      ...
      -- counter period.
      Minimum_Period    : num RANGE 0 .. 16#FFFF#;
      ...
      Page_Protection   : num RANGE 0 .. 16#00FF#;
   END RECORD
   WITH
      Convention => C;
   ...
   FUNCTION Get_APICs(
      MADT_Address : IN System.Address)
