...
static void _c_char_format(const char_ht arg)
{
	printc(arg);
}

static void _s_string_format(const char_ht *arg)
{
	print(arg);
}
...
static void _D_decimal_format(const uint32_t arg)
{
...
static void _h_hexidecimal_format(const int32_t arg)
{
...
static void _H_hexidecimal_format(const uint32_t arg)
{
...
static void _p_pointer_format(const uintptr_t arg)
{
	...
	for (i = 0; string_format[i]; ++i) if (string_format[i] == '%')
	{
		switch (string_format[i + 1])
		{
			...
			case 'p':
				...
				break;
			case 'l':
				//_l_long_decimal_format(va_arg(args, int64_t));
				break;
			case 'L':
				//_L_llong_decimal_format(va_arg(args, uint64_t));
				break;
			case '%':
				printc('%');
				break;
