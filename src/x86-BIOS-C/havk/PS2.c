...
{
	static uint_fast8_t ps2_scan_code;
	static char_ht ps2_key_last, ps2_key_buffer;

	ps2_key_last = PS2_KEY;
	ps2_scan_code = in_byte(PS2_DATA);
	ps2_key_buffer = ps2_scs2_get_key(ps2_scan_code);

	if (!ps2_key_buffer)
		PS2_KEY = ps2_key_last;
	else PS2_KEY = ps2_key_buffer;
}
...
/* TODO: At the moment, the PS/2 buffer seems to be full even when the
/  status register is declaring that there isn't anything in the buffer.
/  The current workaround is to just write bytes and hope for the best.
/  The shorter function just writes and ignores the repeated input byte. */
#ifndef PROPER_KEYBOARD_INITIALIZATION
bool ps2_initialize_keyboard(uint8_t scan_code_set)
{
	...
	command.zero = 0;
	command.first_port_clock = 0; /* Clocks are enabled if zero. */
	...
	/* Flush output while it's full. */
	while (in_byte(PS2_STATUS_REGISTER) & 1)
		in_byte(PS2_DATA);
...
bool ps2_initialize_keyboard(uint8_t scan_code_set)
{
	...
	{
		hprintf("Could not disable PS/2 scan codes (0x%H).", read);
		return false;
	}
...
{
	/* A switch table should generate a jump table, which is faster
	/  than a bunch of if/else statements without one. */
	switch (scan_code)
	{
